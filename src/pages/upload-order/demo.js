...
function Reviews() {
  ...
  useEffect(() => {
    ...
    async function fetchData(url) {
      try {
        ...
        if (contentType && contentType.includes("application/json")) {
          ...
          // Append the new data to the existing reviewsData
    ...
    fetchData(nextPageUrl); // Fetch initial data
  ...
  const loadMore = () => {
    if (nextPageUrl) {
      // Fetch data from the nextPageUrl if it exists
      setNextPageUrl(nextPageUrl); // This should be updated to load the next page correctly
    }
  };
            ...
            <div className="w-full sm:w-1/2 xs:w-full  md:w-1/2  xl:w-2/3 p-4 ">
              <div>
                <h1 className=" font-extrabold text-4xl leading-[3rem] text-[#1E4755]">
                  MyAssignmentHelp <span> Review</span>
                  <br />
                </h1>
                <div className="flex ">
                  <p
                    className="text-xl leading-8  md:w-[100%] sm:w-[100%] pt-9 font-medium"
                    style={{ color: "#1E4755" }}
                  >
                    Still in Two Minds? The Proof is in Numbers ! 38983 genuine
        ...
        <div className="px-2 py-24 lg:px-32 sm:px-6 sm:pt-16  grid-cols-3 justify-between sm:grid-cols-1 ">
          <div className="flex flex-wrap -mx-4">
            {reviewsData.map((item) => (
              <div className="w-full sm:w-1/2 md:w-1/2 xl:w-1/3 p-4" key={item.id}>
                <div className="c-card block bg-white overflow-hidden shadow-[0px_0px_40px_0px_rgba(0,0,0,0.16)]">
                  <div className="p-4">
                    ...
                    >
                      ...
                      <p className="text-right items-end flex-1">
